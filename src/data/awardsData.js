export const awardsData = [
  {
    id: "aw_new_1",
    title: "Visitor's Nominee",
    year: "2025",
    org: "Indian Institutes of Technology (IITs)",
    desc: "Nominated by the Visitor (President of India) for the selection of Professors at IIT Kanpur and other IITs.",
    icon: "fa-building-columns",
  },
  {
    id: "aw_new_2",
    title: "Lead Adviser",
    year: "2024",
    org: "ICSSR, Ministry of Education",
    desc: "Appointed as Lead Adviser to the Monitoring and Advisory Committee for Research Projects on 'Vision Viksit Bharat @2047'.",
    icon: "fa-user-tie",
  },
  {
    id: "aw1",
    title: "Best Paper Award",
    year: "2022",
    org: "12th ICETD, Seoul, South Korea",
    desc: "Awarded for the paper 'Developing Countries, Technical Non-Tariff Measures and South-South Trade: A Structural Gravity Analysis'.",
    icon: "fa-trophy",
  },
  {
    id: "aw_new_3",
    title: "Governor's Nominee",
    year: "2022",
    org: "Jharkhand Public Service Commission (JPSC)",
    desc: "Nominated by the Hon'ble Governor of Jharkhand for the selection of Faculty Positions.",
    icon: "fa-gavel",
  },
  {
    id: "aw3",
    title: "Best Paper Award",
    year: "2020",
    org: "Jaipuria International Management Conference",
    desc: "Awarded in the IT Track for 'Does Bureaucracy Affect the Outcome of Logistics Performance?'.",
    icon: "fa-award",
  },
  {
    id: "aw4",
    title: "Best Paper Award",
    year: "2019",
    org: "14th FIKUSZ Symposium, Obuda University, Hungary",
    desc: "Received Best Paper Award in the English Presentation Category.",
    icon: "fa-scroll",
  },
  {
    id: "aw5",
    title: "Executive Council Member",
    year: "2019 - Present",
    org: "Indian Society of Labour Economics (ISLE)",
    desc: "Serving as an elected member of the Executive Council.",
    icon: "fa-users",
  },
  {
    id: "aw6",
    title: "Best Teacher Award",
    year: "2017",
    org: "Indian Institute of Technology Patna",
    desc: "Honored on Teachers' Day for excellence in teaching and academic contribution.",
    icon: "fa-chalkboard-user",
  },
  {
    id: "aw7",
    title: "Member, Board of Governors",
    year: "2016 - 2018",
    org: "Indian Institute of Technology Patna",
    desc: "Served on the Board of Governors (BoG) and Finance Committee (FC).",
    icon: "fa-landmark",
  },
  {
    id: "aw8",
    title: "Diploma of Appreciation",
    year: "2014",
    org: "University of Bialystok, Poland",
    desc: "Awarded at the IV International Scientific Conference on 'Public Sector in the Modern Economy'.",
    icon: "fa-certificate",
  },
  {
    id: "aw9",
    title: "Bharat Shiksha Ratan Award",
    year: "2012",
    org: "Global Society for Health & Educational Growth",
    desc: "For outstanding contribution to national educational growth.",
    icon: "fa-medal",
  },
  {
    id: "aw10",
    title: "JSPS Fellow",
    year: "2011",
    org: "Japan Society for the Promotion of Science",
    desc: "Awarded the prestigious JSPS Fellowship.",
    icon: "fa-handshake",
  },
];

export const mediaData = [
  // --- FEATURED VIDEOS (Top Priority) ---
  {
    id: "v1",
    type: "video",
    featured: true, // Styling Flag
    date: "Oct 10, 2022",
    title: "Bihar Bihan: Exclusive Interview",
    outlet: "DD Bihar",
    desc: "A deep dive discussion on the economic landscape of Bihar and future prospects.",
    videoId: "Sp2VPypKecI",
  },
  {
    id: "v2",
    type: "video",
    featured: true, // Styling Flag
    date: "Jul 01, 2015",
    title: "Prime Time: E-Learning & NPTEL",
    outlet: "NDTV",
    desc: "Interview with Ravish Kumar on how NPTEL provides solutions to higher education scarcity.",
    videoId: "jV2AVnySgtc",
  },
  // --- MEDIA ARCHIVE (The Rest) ---
  {
    id: "m1",
    type: "news",
    date: "Dec 16, 2025",
    title: "Democratic Education & Global Opportunities",
    outlet: "Prabhat Khabar",
    desc: "Coverage of the lecture on 'New Technologies: Challenges and Opportunities for Dalits'.",
    img: "/Photos/gallery/chared a public session at AN sinha institute, Patna.jpeg",
  },
  {
    id: "v3",
    type: "video",
    date: "Mar 17, 2022",
    title: "Keynote at E-Summit 2022",
    outlet: "Amity University",
    desc: "Delivered the keynote address at the Entrepreneurship Summit.",
    videoId: "lp6VolDHO9M",
  },
  {
    id: "v4",
    type: "video",
    date: "Apr 29, 2022",
    title: "#TalkingTrade: Outcome Driven Policy",
    outlet: "IMPRI",
    desc: "Strategic Framework for Outcome Driven Policy to Transform Manufacturing.",
    videoId: "uRzytzzMdcE",
  },
  {
    id: "v5",
    type: "video",
    date: "Oct 27, 2021",
    title: "Global Value Chain Participation",
    outlet: "IMPRI",
    desc: "Discussion with Prof. Takahiro Sato on India's role in global value chains.",
    videoId: "Av_Ji1xBzDA",
  },
  {
    id: "v6",
    type: "video",
    date: "Aug 20, 2021",
    title: "The Blue Economy & Foreign Trade",
    outlet: "IMPRI",
    desc: "Discussing port-based development and trade expansion with Prof. Mukul Asher.",
    videoId: "TJ-YaUFwiC4",
  },
];

// Preserving your extensive gallery list
export const galleryImages = [
  {
    src: "/Photos/gallery/nalanda_univ_pangaria.webp",
    alt: "With Finance Commission Chairman Arvind Panagariya at Nalanda University",
  },
  {
    src: "/Photos/gallery/arjun_mandal.webp",
    alt: "Sri Arjun Mandal showcasing his President Award to Prof. Nalin Bharti",
  },
  {
    src: "/Photos/gallery/panagariya.webp",
    alt: "Meeting at Rajgir with Finance Commission Chairman Arvind Panagariya",
  },
  {
    src: "/Photos/gallery/Award_conference.webp",
    alt: "Awarding Participant at the GI conference",
  },
  {
    src: "/Photos/gallery/at HNLU Raipur.webp",
    alt: "Group Photo at HNLU Raipur",
  },
  {
    src: "/Photos/gallery/governor_group_photo.webp",
    alt: "Group Photo with honorable Governor of Bihar",
  },
  {
    src: "/Photos/gallery/eastern_regional_conference.webp",
    alt: "16th Finance Commission eastern regional Conference",
  },
  {
    src: "/Photos/gallery/At Nalsar.webp",
    alt: "At NALSAR University",
  },
  {
    src: "/Photos/gallery/viva_kislay.webp",
    alt: "Phd viva voce of Mr kislay kashyap with external prof. Arup Mitra",
  },
  {
    src: "/Photos/gallery/At NIT Patna as a speaker in FDP.webp",
    alt: "Speaker at NIT Patna FDP",
  },
  {
    src: "/Photos/gallery/award ceremony at NIFT PATNA.webp",
    alt: "Award Ceremony at NIFT Patna",
  },
  {
    src: "/Photos/gallery/cnlu patna.webp",
    alt: "Event at CNLU Patna",
  },
  {
    src: "/Photos/gallery/arjun_medicinal plants.webp",
    alt: "Documenting 200+ rare medicinal species with President Awardee Sri Arjun Mandal.",
  },
  {
    src: "/Photos/gallery/special program.webp",
    alt: "at CLH in a special student program",
  },
  {
    src: "/Photos/gallery/nalin_award_governer.webp",
    alt: "Felicitation by Hon. Governor",
  },
  {
    src: "/Photos/gallery/john_taylor_stanford.webp",
    alt: "With Prof. John B. Taylor, Stanford University, renowned for the Taylor Rule in monetary policy.",
  },
  {
    src: "/Photos/gallery/GI_workshop.webp",
    alt: "Workshop on GI at IIT Patna",
  },
  {
    src: "/Photos/gallery/GI_workshop1.webp",
    alt: "Workshop on GI at IIT Patna",
  },

  {
    src: "/Photos/gallery/IHD.webp",
    alt: "IHD Conference, New Delhi",
  },
  {
    src: "/Photos/gallery/FDP at NIT Patna.webp",
    alt: "FDP at NIT Patna",
  },
  {
    src: "/Photos/gallery/India LICS 2025.jpeg",
    alt: "India LICS 2025",
  },
  {
    src: "/Photos/gallery/women_empowerment.webp",
    alt: "CHCDS Women Empowerment program",
  },
  {
    src: "/Photos/gallery/lamp_gi.webp",
    alt: "GI workshop at IIT Patna",
  },
  {
    src: "/Photos/gallery/mips 2024.jpg",
    alt: "MIPS 2024 Conference",
  },

  {
    src: "/Photos/gallery/national statisyics day .jpg",
    alt: "National Statistics Day",
  },
  {
    src: "/Photos/gallery/rajesh_fieldvisit.webp",
    alt: "With Prof. Rajesh Kumar, Professor of Linguistics and English Language, IIT Madras",
  },
  {
    src: "/Photos/gallery/with guest.webp",
    alt: "Guest Talk at IIT Patna",
  },
  {
    src: "/Photos/gallery/dhiresh_iitp.webp",
    alt: "with Prof. Dhiresh Kulshrestha, IIT Patna",
  },
  {
    src: "/Photos/gallery/nlu.webp",
    alt: "International Conference of Law and Economics, NLU",
  },
  {
    src: "/Photos/gallery/round table meet.webp",
    alt: "Round Table Meeting",
  },

  {
    src: "/Photos/gallery/with_senior_faculty.webp",
    alt: "With Senior Faculty",
  },
  {
    src: "/Photos/gallery/Ihd conference.webp",
    alt: "Presenting at IHD Conference",
  },

  {
    src: "/Photos/gallery/nobel_laureate_vernon_smith.webp",
    alt: "With Nobel Laureate Prof. Vernon L. Smith (Economics, 2002) at an academic conference.",
  },
  {
    src: "/Photos/gallery/women seminar.webp",
    alt: "Women Seminar",
  },
];
